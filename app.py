...
if abgeschickt:
    with st.spinner("🔍 Analyse läuft..."):
        st.session_state.analyse_text = gpt_analyse(adresse, baujahr, wohnfläche, heizung, dach, keller, fenster, solar)
    ...
    if st.button("📄 Professionelle PDF erstellen"):
        pdf_buffer = create_pdf(st.session_state.analyse_text, adresse, baujahr, wohnfläche)
    ...
    # Projekt speichern nach Analyse
    projekt = {
        "adresse": adresse,
        "baujahr": baujahr,
        ...
        "fenster": fenster,
        "solar": solar,
        "analyse": st.session_state.analyse_text
    }

    if projekt not in st.session_state.projekte:
        st.session_state.projekte.append(projekt)

    # Projektverlauf anzeigen
    if len(st.session_state.projekte) > 1:
        st.markdown("### 📁 Vorherige Projekte")
        projekte_dict = {f"{i+1}. {p['adresse']} ({p['baujahr']})": i for i, p in enumerate(st.session_state.projekte)}
        auswahl = st.selectbox("Wähle ein Projekt zur Anzeige", list(projekte_dict.keys()))
        ausgewaehlt = st.session_state.projekte[projekte_dict[auswahl]]
        
        with st.expander("📄 Analyse anzeigen"):
            st.markdown(ausgewaehlt["analyse"])
